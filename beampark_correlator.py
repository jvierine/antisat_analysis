...
def vector_diff_metric(t, r, v, r_ref, v_ref, **kwargs):
    ...
    num = kwargs.get('measurement_num')
...
def sorting_function(metric):
    ...
    '''
    P = np.abs(metric['dr']) + 2*np.abs(metric['dv'])
    return np.argsort(P, axis=0)


def metric_reduce(total_metric, next_metric):
    '''Reduce function used on the cumulative metric and the next measurnment. Use simple summation.
    '''
    return total_metric + next_metric
